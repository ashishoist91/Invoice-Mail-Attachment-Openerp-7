...
class email_template(osv.osv):
    ...
    def get_invoice_attachments(self, cr, uid, template_id, res_id, context=None):
        attachment_ids = self.pool.get('ir.attachment').search(cr, uid, [('res_model','=','account.invoice'),('res_id','in',[res_id])], context=context)
    ...
    def generate_email(self, cr, uid, template_id, res_id, context=None):                   
        ...
        if template.send_invoice_attachments:
            invoice_attachment_ids = self.get_invoice_attachments(cr, uid, template_id, res_id, context=context)
            values['attachment_ids'].extend(invoice_attachment_ids)
        return values
